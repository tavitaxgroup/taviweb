# Prompt Studio AI Auto Garage - Dùng Ngay

Copy toàn bộ prompt này vào Studio AI khi muốn chuyển thiết kế landing page garage sửa chữa ô tô thành code Next.js.

```txt
Hãy chuyển thiết kế landing page garage sửa chữa ô tô này thành code Next.js.
...
7. Template component cần tạo là AutoGarageTemplate.
8. Template key là auto-garage.
9. Ngành là garage sửa chữa ô tô.
...
Mục tiêu:
Tạo một website template public cho doanh nghiệp thuộc ngành garage sửa chữa ô tô.
...
Template cần tạo:
- template_key: auto-garage
- template component: AutoGarageTemplate
...
6. DemoTemplateRenderer render AutoGarageTemplate.
...
Ví dụ sai:
<h1>Garage Minh Phát</h1>
<img src="/auto-garage.jpg" />
...
- Không dùng ảnh tai nạn gây sốc.
- Không hứa sửa mọi lỗi hoặc cam kết kỹ thuật quá mức.
...
Template này render website demo public cho garage sửa chữa ô tô tại /demo/[place_id]. Hiện tại dùng mock data để preview, sau này thay bằng Supabase fetch.
Code template kỹ thuật, đáng tin, thực dụng, tối ưu CTA gọi kiểm tra xe.
AutoGarageTemplate dùng 10 section chuẩn: Navbar, Hero, Trust, About, Services, Gallery, Why Choose Us, Reviews, Contact CTA, Footer.
Bind business, hero, trust, about, services, gallery, reviews, contact, seo.
Không hard-code tên garage, phone, ảnh, giá dịch vụ, hãng xe hoặc cam kết sửa mọi lỗi.
Mobile-first; trên mobile ưu tiên nút gọi ngay và xem đường đi.
Desktop dùng layout mạnh, rõ, có cảm giác kỹ thuật và đáng tin.
Không dùng video background; gallery lazy load.
Thiếu ảnh dùng fallback garage/xưởng xe an toàn.
Thiếu phone dùng Facebook/map/email.

Service mặc định nên phù hợp garage:
- Bảo dưỡng định kỳ.
- Kiểm tra tổng quát.
- Sửa chữa động cơ.
- Điện/điện lạnh ô tô.
- Lốp, phanh và hệ thống an toàn.

Visual direction:
- Màu chính: graphite/charcoal.
- Accent: cam kỹ thuật hoặc steel gray.
- Hình ảnh: xưởng xe, kỹ thuật viên, cầu nâng, dụng cụ, xe đang kiểm tra.
- Tránh ảnh tai nạn nặng, hình quá tối hoặc cảm giác showroom sang trọng không phù hợp garage sửa chữa.

CTA direction:
- CTA chính: Gọi kiểm tra xe.
- CTA phụ: Xem đường đi hoặc Nhắn Facebook.
...
Kết quả mong muốn:
Tạo một Next.js public website template cho ngành garage sửa chữa ô tô.
