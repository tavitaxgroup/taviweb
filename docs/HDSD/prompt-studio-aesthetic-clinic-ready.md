# Prompt Studio AI Thẩm Mỹ Viện - Dùng Ngay

Copy toàn bộ prompt này vào Studio AI khi muốn chuyển thiết kế landing page thẩm mỹ viện thành code Next.js.

```txt
Hãy chuyển thiết kế landing page thẩm mỹ viện này thành code Next.js.
...
7. Template component cần tạo là AestheticClinicTemplate.
8. Template key là aesthetic-clinic.
9. Ngành là thẩm mỹ viện.
...
Mục tiêu:
Tạo một website template public cho doanh nghiệp thuộc ngành thẩm mỹ viện.
...
Template cần tạo:
- template_key: aesthetic-clinic
- template component: AestheticClinicTemplate
...
6. DemoTemplateRenderer render AestheticClinicTemplate.
...
- Không có before-after thì không render before-after.
...
Template này render website demo public cho thẩm mỹ viện tại /demo/[place_id]. Hiện tại dùng mock data để preview, sau này thay bằng Supabase fetch.
Code template sang trọng, an toàn về claims, dữ liệu truyền qua DemoPageData props.
AestheticClinicTemplate dùng 10 section chuẩn: Navbar, Hero, Trust, About, Services, Gallery, Why Choose Us, Reviews, Contact CTA, Footer.
Bind business, hero, trust, about, services, gallery, reviews, contact, seo.
Không hard-code kết quả thẩm mỹ, tên doanh nghiệp, phone, ảnh.
Mobile-first; desktop spacing rộng, CTA luôn visible.
Không dùng video background; gallery lazy load.
Không có before-after thì không render before-after.
Thiếu ảnh dùng fallback clinic cao cấp.
...
Kết quả mong muốn:
Tạo một Next.js public website template cho ngành thẩm mỹ viện.
